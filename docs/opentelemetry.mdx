...
The diagnostics provider's `boot()` tries to load `@adonis-agora/diagnostics/otel` and start it. If `@opentelemetry/api` can't be resolved (no OTel SDK installed), the import fails and the bridge is skipped silently. If it resolves, the bridge starts and is torn down on `shutdown()`.
...
| --- | --- | --- | --- |
| `tracerName` | `string` | `@adonis-agora/diagnostics` | Name passed to `trace.getTracer(...)`. |
